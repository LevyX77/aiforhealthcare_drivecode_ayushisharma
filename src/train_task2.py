...
MODEL_SAVE_PATH = os.path.join(HOME, "Alzheimer_Project", "models", "task2_model.pth")
...
class MediumResNet(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(64, 2) # Important: 2 classes (CN, MCI)
        )
...
# --- TRAIN ---
def train():
    print("STARTING TASK 2 (CN vs MCI)...")
    if torch.cuda.is_available(): torch.cuda.empty_cache()

    # Data
    df = pd.read_csv(CSV_PATH)
    mapping = {'CN': 0, 'MCI': 1} # Ignoring AD
    ...
    counts = [y_train.count(i) for i in [0, 1]]
    ...
    train_transforms = Compose([
        ...
        RandAffined(keys=["image"], prob=0.3, rotate_range=(0.1), scale_range=(0.1)),
