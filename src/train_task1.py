...
MODEL_SAVE_PATH = os.path.join(HOME, "Alzheimer_Project", "models", "task1_model.pth")
...
class MediumResNet(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(64, 2) # Important: 2 classes (CN, AD)
        )
...
# --- TRAIN ---
def train():
    print("STARTING TASK 1 (CN vs AD)...")
    if torch.cuda.is_available(): torch.cuda.empty_cache()

    # Data
    df = pd.read_csv(CSV_PATH)
    mapping = {'CN': 0, 'AD': 1} # Ignoring MCI
